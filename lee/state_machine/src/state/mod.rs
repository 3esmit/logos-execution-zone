...
impl V03State {
    ...
    /// Returns every deployed program identifier in ascending order.
    #[must_use]
    pub fn program_ids(&self) -> Vec<ProgramId> {
        let mut program_ids = self
            .public_state
            .iter()
            .filter(|(_, account)| account.program_owner == PROGRAM_STORAGE_OWNER)
            .map(|(account_id, _)| ProgramId::from(*account_id))
            .collect::<Vec<_>>();
