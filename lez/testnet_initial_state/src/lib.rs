...
fn initial_priv_accounts_private_keys() -> Vec<PrivateAccountPrivateInitialData> {
    ...
    vec![
        PrivateAccountPrivateInitialData {
            account: Account {
                program_owner: DEFAULT_PROGRAM_OWNER.into(),
...
mod tests {
    ...
    fn development_fixture_profile_contains_development_pinata_state() {
        ...
        assert_eq!(
            programs::pinata().id(),
            ProgramId::from(expected_pinata_account.program_owner),
        );
        assert_eq!(
            system_accounts::pinata_account_id(),
        ...
        assert!(
            state
                .program_ids()
                .contains(&ProgramId::from(expected_pinata_account.program_owner))
        );
        assert_eq!(
