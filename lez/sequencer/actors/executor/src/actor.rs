...
use crate::{
    ...
    protocol::{
        ...
        GetGenesisBlockId, GetLastBlockId, GetProgramIds, GetProofsAndRoot, GetTransaction,
...
impl<BP: BlockPublisherTrait + Send + 'static> Message<GetGenesisBlockId> for ExecutorActor<BP> {
    type Reply = BlockId;

    async fn handle(
        &mut self,
        GetGenesisBlockId: GetGenesisBlockId,
        _ctx: &mut Context<Self, Self::Reply>,
    ) -> Self::Reply {
        self.sequencer.block_store().genesis_id()
    }
}
