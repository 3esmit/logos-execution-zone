{
  ...
  inputs = {
    logos-liblogos.url = "github:3esmit/logos-liblogos?rev=c6cca136cbac6bd1f413a31f1bc6370262e190ad";
    ...
    in
    {
      packages = forAll (
        ...
        let
          ...
          # Download from the static crates.io host; the checksum from Cargo.lock
          # is the sha256 of the .crate file, so this is a verified fixed-output fetch.
          risc0CircuitRecursionCrate = pkgs.fetchurl {
            url = "https://static.crates.io/crates/risc0-circuit-recursion/risc0-circuit-recursion-${risc0CircuitRecursion.version}.crate";
